<?php

/**
 * @copyright   (c) 2014 brian ridley
 * @author      brian ridley <ptlis@ptlis.net>
 * @license     http://opensource.org/licenses/MIT MIT
 *
 * PHP Version 5.3
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace ptlis\ConNegBundle\Test;

use Symfony\Bundle\FrameworkBundle\Test\WebTestCase;

class EncodingBestTest extends WebTestCase
{
    public function testEncodingBestOne()
    {
        $client = static::createClient();

        $client->request(
            'GET',
            '/encoding_best',
            array(),
            array(),
            array('HTTP_ACCEPT_ENCODING' => '*')
        );

        $this->assertSame(
            'gzip',
            $client->getResponse()->getContent()
        );
    }


    public function testEncodingBestTwo()
    {
        $client = static::createClient();

        $client->request(
            'GET',
            '/encoding_best',
            array(),
            array(),
            array('HTTP_ACCEPT_ENCODING' => 'deflate;q=0.7,gzip;q=0.9,compress;q=0.2')
        );

        $this->assertSame(
            'gzip',
            $client->getResponse()->getContent()
        );
    }


    public function testEncodingBestThree()
    {
        $client = static::createClient();

        $client->request(
            'GET',
            '/encoding_best',
            array(),
            array(),
            array('HTTP_ACCEPT_ENCODING' => 'gzip;q=0.1,deflate,compress;q=0.5')
        );

        $this->assertSame(
            'deflate',
            $client->getResponse()->getContent()
        );
    }


    public function testEncodingBestFour()
    {
        $client = static::createClient();

        $client->request(
            'GET',
            '/encoding_best',
            array(),
            array(),
            array('HTTP_ACCEPT_ENCODING' => '')
        );

        $this->assertSame(
            'gzip',
            $client->getResponse()->getContent()
        );
    }
}
